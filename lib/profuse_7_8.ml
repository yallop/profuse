...
module Out = struct
  ...
  module Open = struct
    ...
    module Flags = struct
      ...
      let to_uint32 { direct_io; keep_cache } =
        Unsigned.UInt32.logor
          (if direct_io  then T.fopen_direct_io else Unsigned.UInt32.zero)
          (if keep_cache then T.fopen_keep_cache else Unsigned.UInt32.zero)
